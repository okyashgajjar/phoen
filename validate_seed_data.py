...
Validates all 16 CSV seed files for relational integrity, pricing consistency,
inventory validity, date formats, SKU uniqueness, and business logic.
"""
...
def validate():
    ...
    for r in srv_rows:
        ...
        try:
            ...
            price = float(r[6])
            tax = float(r[7])
            if cost < 0 or price < 0 or cost > price:
                invalid_prices += 1
            if tax < 0 or tax > 100:
                invalid_taxes += 1
        except ValueError:
    ...
    total_issues = (
        ...
        invalid_taxes + invalid_tiers + invalid_categories
    )
